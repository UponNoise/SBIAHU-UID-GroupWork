...
| IV 加载必须检查道路合法性 | 已满足 | 点击点投影到道路中心线，距离超出道路宽度则报错 |
...
| OR5 Path planning | 已补齐 | 两个任意地图点先吸附到最近道路点，再用自定义 Dijkstra 规划最短路 |
...
4. 增加 `validate_navigation_core.m`，覆盖地图尺寸、坐标换算、道路合法性、吸附、路径规划和局部遮罩。
5. 增加 `validate_ui_smoke.m`，用于快速检查 UI 创建阶段是否抛出异常。

## 已知限制

- 道路模型是人工中心线近似，不是从图片自动精确分割道路边界。
